 ...
 * 
 * 
 * 通过开发时入口模块为起点，根据 require，import等，递归分析出所有依赖关系，
 * 
...
//- - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -//
    ...
    入口 ----> 入口模块文件 -------> 已记录则结束 ---> 读取模块内容 --> AST( 抽象语法树 ) ---> 将依赖关系保存 ------------>>
...
loader 的功能定位是转换代码，而一些其他的操作难以使用 loader 完成
...
/**
 * loader                   // 本质上是一个函数
 *                          // loader 仅作用于 webpack 打包过程中，必须使用 commonjs 模块化语法
 * 
 * 
...
... ---> 读取模块内容 ---> execute-loaders ---> AST( 抽象语法树 ) ---> ....

                        // loader 运行的位置分析 
                        // 出现在读取模块内容之后，抽象语法树分析之前
                        // 会与模块内容合并后，再进行抽象语法树分析
 ...
 */
    ...
    | 初始化 -------------------> 编译 ---------------------> 输出 |    |    |
        ...
        * 当 webpack 开启监听模式，文件发生变化时，会重新创建一个 compilation 用于重新打包输出文件
 ...
 * 
 * 
 * apply(); 仅在 webpack 创建完 compiler 后执行一次用于加载事件           // 类似于 window.onload
 */
...
2) eval-source-map              // 建议使用( 打包快，可精确定位错误位置 )
...
- hidden-source-map
    ...
    与 source-map 相同，但不会为 bundle 添加引用注释。如果你只想 source map 映射那些源自错误报告的错误堆栈跟踪信息
    但不想为浏览器开发工具暴露你的 source map，这个选项会很有用与 source-map 相同，但不会为 bundle 添加引用注释
...
// 根据配置不同的环境变量，将 webpack.config.js 模块化

npx webpack --env dev                                   // return env: "dev"

npx webpack --env pro                                   // return env: "pro"
...
|- webpack.config.js
|-
|- webpack.base.js              // comon config
|- webpack.dev.js               // development config
|- webpack.pro.js               // produce config
|
|
...
module.exports = function(env) {
    ...
    if(env && env === "dev"){
        ...
        return require('./webpack.dev.js ');            // 根据不同的环境变量，返回不同环境的 配置文件
    
    } else {
    
        return require('./webpack.pro.js ');            // 根据不同的环境变量，返回不同环境的 配置文件
    }
}
 ...
 * 
 * 
 * 该配置影响 入口 和 loaders 解析，入口 和 loaders 的相对路径会以 context 的配置作为 [基础路径]
 */
...
module.exports = {
    ...
    entry: {
        ...
        index './index.js',             // 当配置了 context 时，解析 entry & loader 
                                        // './index.js' === './[context]/index.js'
    },
...
module.exports = {
    ...
    output: {
        ...
        libraryTarget: 'type'               // 设置以 什么方式 暴露给外部 ( var, window, this, global, commonjs ... )
    }
};


// 将 webpack 打包的文件，以指定的方式暴露到外部( 默认为 var )

var = finallyResult = [webpack.export]
